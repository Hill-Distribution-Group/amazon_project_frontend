  ...
  const ResultTable = ({ data, setData, onSaveSelected, isSavedResults = false, onRemoveSelected, onDecisionUpdate, onCommentUpdate, users, assignees, onAssigneeChange }) => {
      ...
      if (commentProductIndex !== null) {
        ...
        if (onCommentUpdate) {
          ...
          const newData = data.map((item, index) => 
            index === commentProductIndex ? updatedItem : item
          );
          setData(newData);
        }
        handleCommentClose();
      }
    }, [data, setData, commentProductIndex, currentComment, onCommentUpdate]);

    const filteredData = useMemo(() => {
      if (!data || data.length === 0) return [];
      return data.filter(product => 
        (product["Search Query"] || '').toLowerCase().includes(filterText.toLowerCase()) || 
    ...
    const displayColumns = useMemo(() => {
      return [
        "Search Query", // Renamed and moved to the beginning
        "ASIN",
        "Amazon Product Name",
        "Matching Percentage",
        "Sales Volume",
          ...
          <TableBody>
            {sortedData.map((product, index) => (
              <TableRow 
                ...
                onContextMenu={isSavedResults ? (e) => handleContextMenu(e, index) : undefined}
